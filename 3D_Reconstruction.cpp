...
int main(void)
{
	...
	Size size = Size(640*2,480); // 입력받는 이미지 그대로 저장이 아님
	Mat black_img(480, 640, CV_8UC3, Scalar(0, 0, 0));

	//파일로 동영상을 저장하기 위한 준비  
	VideoWriter outputVideo;
	outputVideo.open("FR_3D_reconstuction.avi", VideoWriter::fourcc('X', 'V', 'I', 'D'),
		15, size, true);
	...
	while (1){
		...
		cv::hconcat(imgr, black_img, imgr);
...
//			ref_img = imgr;
	...
	while (1){
		...
		*/
		...
		cv::hconcat(imgt, black_img, imgt);
