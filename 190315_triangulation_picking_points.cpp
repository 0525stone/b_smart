// 2019.03.14 화면에 8개 8개 점 찍으면 다 계산해서 나옴
...
int main(void)
{
	...
	double scale = 0.1;
	printf("load ? \n");
	cv::Mat image1 = cv::imread("data/sample07.jpg");	// 내가 찍어서 해볼랬는데 잘안된다... 원인찾기
	...
	while (points1.size() < N_point)  // 열두번을 찍는거
	{
	...
	*/
		...
		//	for (int i = 0; i <1; i++)
	{
		...
		// Mat의 요소 접근이 잘못된듯... -> matrV.at<double>(0, 0)

		// vector에다가 하나의 벡터 추가를 어케하냐고.... -> push_back으로 틀 맞춰서
